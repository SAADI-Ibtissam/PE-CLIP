...
def fmix(data, targets, alpha, decay_power, shape, max_soft=0.0, reformulate=False):
    ...
    shuffled_data = data[indices].cuda()
    shuffled_targets = targets[indices].cuda()
    x1 = torch.from_numpy(mask).cuda()*data.cuda()
    x2 = torch.from_numpy(1-mask).cuda()*shuffled_data.cuda()
